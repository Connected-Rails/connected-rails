//! Volumetric clouds (plan 14.1).
//!
//! The clouds are raymarched the way Guerrilla's Nubis does it — a shape noise
//! eroded by a detail noise, Beer-Powder attenuation, a forward-scattering phase
//! for the silver lining — but **not per screen pixel**. The march writes a
//! 512 × 256 equirectangular panorama, and a dome samples it. That is 131 k
//! pixels a frame whatever the window is, against two million at 1080p, and it is
//! the whole performance argument: a camera that stands on the ground never
//! enters a cloud, so a direction is all a cloud has to be a function of.
//!
...
/// Panorama size. 768 × 384 is a quarter of a degree per texel; below that a
/// cumulus at ten kilometres draws as a staircase however well the dome filters
/// it. Still a fifth of a 1080p screen, which is the whole point.
const PANORAMA: UVec2 = UVec2::new(768, 384);
...
fn spawn(
    ...
    commands.spawn((
        ...
        Camera {
            order: -1,
            // Nothing behind the clouds: what the march does not cover has to
            // come out as no cloud at all, not as a background colour.
...
/// Feeds the march from the sky and keeps the dome on the camera.
...
fn update(
    ...
    let params = CloudParams {
        ...
        // quite black, or a night cloud would be a hole in the stars.
        ...
        ambient: (Vec3::new(0.50, 0.62, 0.85)
            * crate::sky::SUN_ILLUMINANCE
            * (0.10 * (0.05 + 0.95 * daylight) + 0.9 * sky.flash))
            .extend(weather.cover),
        layer: Vec4::new(
            weather.base,
            // A closed deck is a thick one; a fair-weather cumulus is not.
            600.0 + 1_400.0 * weather.cover,
            -weather.bearing.sin() * weather.wind,
            weather.bearing.cos() * weather.wind,
        ),
    };
