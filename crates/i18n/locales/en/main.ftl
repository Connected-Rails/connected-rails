...
snd-quantity-vigilance-alert = Vigilance device (Sifa) sounding
snd-quantity-protection-alert = Train protection (PZB/LZB) horn
snd-quantity-dynamic-brake = Dynamic brake force [kN]
...
sig-motion-profile = Motion
sig-motion-linear = linear
sig-motion-semaphore = mechanical semaphore
sig-fall-seconds = fall
sig-fall-seconds-hint = gravity-driven time from full travel to the rest stop
sig-rebound = rebound
sig-rebound-hint = coefficient of restitution at the mechanical stop — 0 has no bounce
sig-mount = Mount
...
help-draw = Lay track: press and drag sets start and heading, every click adds a piece · Ctrl straight · Enter or right-click finishes · Esc cancels
...
tool-select = Select
tool-select-hint = Click picks a single thing — the track it would pick lights up under the cursor · Ctrl+click gathers a multi-selection, a second Ctrl+click takes it out again · press on empty ground and drag to select in a circle · a selected track shows its round handles, and a free start its move/turn gizmo (W/E) · Delete removes what is selected
tool-draw = Lay track
...
tool-split-hint = Click a track to cut it there — two tracks on one joint, each selectable and deletable on its own
tool-join = Join ends
tool-join-hint = Click one open end, then another. Ends on one spot are welded; ends apart are staked out like Zusi's calculator: transitions – arc – transitions plus one compensating straight, or a double arc with an intermediate straight where no single arc reaches
tool-offset = Parallel track
tool-offset-hint = Click a track to lay its parallel at the set spacing, on the side of the click
tool-crossover = Crossover
tool-crossover-hint = Click the track it leaves, then the parallel track it reaches — both are cut and wired into the two turnouts of a crossover
tool-gradient = Gradient
tool-gradient-hint = Click a track to put a gradient break point there. Take hold of a break point and move the mouse up or down the screen to lift or lower it — the stretches on either side change their per mille, every other point stays. The selection panel edits the figures
tool-device = Place device
tool-device-hint = Click a track to put the chosen device kind on it
tool-area = Mark area
...
tool-object-hint = Click a track to drop the chosen 3D object at its predefined offset and rotation
tool-tree = Place tree
tool-tree-hint = Every click plants one tree of the chosen species — anywhere, free of the track
tool-forest = Forest brush
tool-forest-hint = Clicks outline an area; Enter or right-click fills it with single trees — each one stays individually editable and deletable · Esc cancels
tool-brush = Marking brush
tool-brush-hint = Hold the left button and sweep to mark trees and objects in bulk; Delete removes them together, Esc clears the marking
tool-marker = Place marker
tool-marker-hint = Every click sets a reference marker in the named layer — a drawing aid, not equipment: nothing in the simulation reads it
marker-layer = Layer
...
tool-terrain-raise = Raise ground
tool-terrain-raise-hint = Every click stamps one round stroke that lifts the ground by the set amount. The track keeps its height: strokes work the ground, the cutting and embankment are laid over them afterwards
tool-terrain-lower = Lower ground
tool-terrain-lower-hint = The same stroke, downward — a hollow, a pond bed, a pit
tool-terrain-level = Flatten
tool-terrain-level-hint = Pulls the ground inside the circle to the height it has under the click — the World Editor's plateau gesture
tool-terrain-rail = Level to rail
tool-terrain-rail-hint = Pulls the ground to the height of the nearest rail — a station forecourt, a depot, a level yard
status-no-ground-height = No ground tile under the click yet — wait for the tiles to build, or click nearer the line
...
terrain-amount-hint = Metres the centre moves — up or down is the tool in hand
...
# --- Laying track ----------------------------------------------------------
# The properties of the next piece — the World Editor's panel for the piece
# about to be laid, which never touches one already lying there.
lay-type = Track type
lay-type-hint = What the next piece is built as — texture, roughness and superstructure limit come from track_types/*.ron of a mod; the content drawer picks it too
lay-speed = Permitted speed
lay-speed-hint = Speed profile the next piece starts with; 0 leaves the line's default
lay-grade = Gradient
lay-grade-hint = The next piece climbs (+) or falls (−) at this rate; the gradient tool re-shapes it afterwards
lay-power = Electrification
lay-power-unset = (as the line)
lay-power-hint = What hangs over the next piece; unset says nothing and the line's default applies
lay-formation = Ballast bed & embankment
lay-formation-hint = On (default), the terrain builds the formation under the piece — ballast bed, embankment or cutting. Off, only the rails stand: for track on bridges, platforms or ground you shape yourself
lay-parallel = Tracks per lay
lay-parallel-hint = Lays this many parallel tracks at once, spaced to the right of the drawn one — a yard in one gesture
lay-spacing = Track spacing
lay-spacing-hint = Centre distance between parallel tracks; 4 m is the German main-line standard
lay-snap-radius = Snap to standard radii
...
# The join tool's parameters, after Zusi's Absteckrechner: what the connection
# between two open ends is allowed to be built from.
stake-speed = Design speed
stake-speed-hint = Transition lengths and cant follow this speed; 0 takes the speed of the lay options
stake-radius = Radius
stake-radius-hint = Fixed arc radius; 0 lets the calculator grow it until exactly one compensating straight remains — Zusi's automatic
stake-easements = Transition curves
stake-easements-hint = A clothoid at every change of curvature; switched off, the chain is bare straights and arcs
stake-easement-length = Transition length
stake-easement-length-hint = Fixed length per transition; 0 takes the rulebook's cant ramp for the design speed
stake-cant = Cant
stake-cant-hint = The rulebook's cant under the arcs, ramped over the transitions
stake-min-straight = Intermediate straight
stake-min-straight-hint = Shortest straight a double arc keeps between its two arcs
status-stake-not-plausible = No connection fits these two ends — not plausible; move an end or turn it
status-stake-radius-too-big = The chosen radius is too big for the gap between the ends
status-stake-arc-too-short = The arc between the transitions comes out too short — smaller radius or shorter transitions
status-stake-double-impossible = No double arc fits either — move the ends, or change radius and intermediate straight
join-first-set = First end picked — click the end to connect it to.
crossover-first-set = First track cut — click the parallel track the crossover reaches.
draw-aiming = Drag sets the heading — release, then click points
draw-aim-facing = Facing turnout: the branch leaves with the track — release to fix, drag backwards for trailing
draw-aim-trailing = Trailing turnout: the branch runs back along the track — release to fix, drag forwards for facing
draw-readout-arc = { $length } m · R { $radius } m
draw-readout-arc-canted = { $length } m · R { $radius } m · u { $cant } mm
draw-readout-straight = { $length } m · straight
hover-edge-start = Track start · heading { $heading }°
grade-point-readout = h { $height } m · { $before } ‰ → { $after } ‰
grade-at-readout = { $grade } ‰ · h { $height } m
draw-active = Laying: { $segments } pieces — Enter or right-click finishes, Esc cancels
draw-branch = Branch track: { $segments } pieces — Enter or right-click wires the switch, Esc cancels
status-no-open-end = No open track end near the click — the join tool welds buffers
status-join-same-end = Both clicks hit the same end — pick two different ones
status-crossover-same-track = Both clicks hit the same track — the crossover needs the parallel one
status-crossover-not-parallel = The second track does not run parallel at that distance — the turnout arcs land beside it
...
sel-none = Nothing selected — the select tool picks whatever stands on the map, in every category.
sel-edge-summary = Track { $index }: { $length } m, { $segments } segments
sel-edge-devices = { $devices } devices on this track
sel-edge-handles = Drag the round handles on the map to bend the track. With a free start, the gizmo moves the whole track (W) or turns it about its start (E).
...
sel-edge-formation = Ballast bed & embankment
sel-edge-formation-hint = Off, the terrain leaves this track alone — no ballast bed mesh, no embankment, no gravel strip. For track you set on your own constructions; split the edge if only a stretch floats
sel-track-type = Track type
sel-track-type-none = No track type — the track has to name one, nothing stands in for it.
...
sel-grade = Gradient
sel-grade-none = Level over the whole track.
sel-grade-from = From this distance along the track
sel-grade-hint = A break point of its own — from its position on, the track climbs or falls at the new rate
sel-grade-climb = Climb over this track: { $climb } m
action-add-grade-section = Add gradient section
...
track-type-none-installed = No installed mod defines a track type
...
signal-label = { $designation } · { $kind } (device { $device }, index { $index })
sig-designation = Signal designation
sig-designation-hint = Unique operational name printed on the signal plate, e.g. A, N1 or 47P3
sig-interlocking = Interlocking
sig-interlocking-hint = Name of the signal box/interlocking which controls this signal
sig-kind = Kind
...
sig-addons = Subsidiary signals
sig-addons-hint = Equipment physically fitted to this signal. Use + for multiple route-dependent letters or numbers shown by the same theatre display.
sig-addons-not-applicable = These subsidiary-signal fittings are not valid on shunting signals or track locks.
sig-addons-invalid = This selection contains an invalid function, an empty value, or Zs 13 together with a Zs 3 code 1–3.
sig-fitted = fitted
sig-zs-form = fixed board
sig-zs-light = light indicator
sig-zs-a-lights = three lights (A)
sig-zs-blink-light = one flashing light
sig-zs-light-strip = flashing light strip
sig-zs-values = identifying letter, e.g. K
sig-zs-add-letter = Add another route-dependent identifying letter
sig-zs-add-value = Add another route-dependent code
sig-zs1 = Zs 1 substitute signal
sig-zs1-hint = Three white lights forming an A or one flashing white light, at a main signal at stop
sig-zs2 = Zs 2 route indicator
sig-zs2-hint = White illuminated letter; use + to add further possible letters
sig-zs2v = Zs 2v route pre-indicator
sig-zs2v-hint = Yellow illuminated letter on a distant or distant-function signal
sig-zs3 = Zs 3 speed indicator
sig-zs3-hint = White number, multiplied by ten for km/h. A fixed board has one code; a light display may have several
sig-zs3v = Zs 3v speed pre-indicator
sig-zs3v-hint = Yellow number on a distant or distant-function signal
sig-zs6 = Zs 6 opposite-track indicator
sig-zs6-hint = White rising diagonal with bent ends, as a fixed board or light indicator
sig-zs7 = Zs 7 caution signal
sig-zs7-hint = Three yellow lights forming a V at a main signal
sig-zs8 = Zs 8 opposite-track substitute signal
sig-zs8-hint = Three flashing white lights forming an A or a flashing white light strip
sig-zs12 = Zs 12 M board
sig-zs12-hint = White board with red border and a red script M on a main signal
sig-zs13 = Zs 13 terminal/short-entry indicator
sig-zs13-hint = Yellow sideways T; do not combine with Zs 3 code 1, 2 or 3
sig-zs103 = Zs 103 diamond board (DV 301)
sig-zs103-hint = Historic black board with white diamonds on a main signal; only for DV-301 scenarios
...
veg-species = Species
veg-species-hint = A mod's 3D object (objects/*.ron), or a stand its `stand-…` tags describe — the forest brush mixes the stand's species; the placeholder is the app's built-in tree
veg-placeholder = (Placeholder tree)
veg-stands = Stands
veg-single = Single species
tree-yaw = Rotation
...
road-import-tracks-hint = Also import highway=track — the farm roads an agricultural module is stitched with. They come in unpaved unless the mapper wrote a surface or tracktype=grade1
...
road-preset = Road type
road-preset-hint = Which class of the German road system the next one is — it stamps width, surface and markings, and all three stay editable
road-preset-motorway-3 = Autobahn, 3 lanes + shoulder
road-preset-motorway = Autobahn, 2 lanes + shoulder
road-preset-motorway-concrete = Autobahn, 2 lanes + shoulder (concrete)
road-preset-federal = Bundes-/Landstraße, outside towns
road-preset-federal-solid = Landstraße with overtaking ban
road-preset-secondary = Kreisstraße
road-preset-residential = Gemeindestraße
road-preset-residential-narrow = Anliegerstraße
road-preset-living = Spielstraße
road-preset-service = Wirtschaftsweg (asphalt)
road-preset-farm-gravel = Feldweg (gravel)
road-preset-farm-concrete = Feldweg (concrete slabs)
road-preset-path = Fußweg
road-name = Name
road-name-hint = What the road is called in the file and this panel; the OSM import fills it from the map
road-width = Width
road-width-hint = Kerb to kerb — the preset's value is a planning one, and the module may disagree
road-surface = Surface
road-surface-hint = What the carriageway is drawn with; the OSM import reads it from surface=* and tracktype=*
road-surface-asphalt = Asphalt
road-surface-concrete = Concrete
road-surface-gravel = Gravel
road-center-line = Centre line
road-center-line-hint = What runs down the middle, per the RMS: dashed 6-and-12 outside towns, 3-and-6 inside them, or the solid line that never lifts
road-edge-lines = Edge lines
road-edge-lines-hint = The solid white Randlinien along the kerbs — a paved road carries them, an unpaved track does not
road-length = Length: { $length } m
road-count = { $roads } roads on the module
road-active = { $points } points set — Enter or a right click paves the road
sel-road-summary = Road { $index }
sel-road-named = { $name } — road { $index }
status-road-points = A road needs at least two points
...
action-reset = Reset
action-browse-drawer = Pick in the drawer…
status-issues = Findings: { $count }
status-issues-hint = The rule check’s findings — the click opens them under the module category
kind-signal = Signal
...
view-north = Face north
view-north-hint = Face north — camera heading { $degrees }°
view-top-down = Top-down view
view-top-down-hint = Look straight down for track work over the imagery; click again for the free view
view-panel = Properties panel
...
check-signal-designation = Signal { $signal }: signal designation is missing
check-signal-interlocking = Signal { $signal }: controlling interlocking is missing
check-signal-addons = Signal { $signal }: subsidiary-signal equipment is invalid for this function or combination
check-boundary-invalid = Boundary { $boundary }: node is missing or not a buffer
check-missing-track-type = Track { $edge }: names no track type — nothing stands in for one, and the line will not compile
check-unknown-track-type = Track { $edge }: names a track type no installed mod has
check-road-short = Road { $road } has fewer than two points
...
set-volumetric-clouds = Volumetric clouds
set-volumetric-clouds-hint = Marches the cloud deck as a volume — billows, a lit interior and a silver lining where the sun is behind it. Off draws the same clouds as one lit sheet, just as sharp and about a twentieth of the cost.
...
set-aa-4x = 4 ×
set-aa-8x = 8 ×
set-upscaling = Upscaling
set-upscaling-hint = Draws the world below the screen's resolution and reconstructs the full picture from it — the biggest lever on weak hardware. FSR 3 runs on any GPU; DLSS needs an NVIDIA RTX card and a simulator built with it.
set-upscaling-off = Off
set-upscaling-fsr = FSR 3
set-upscaling-dlss = DLSS
set-upscaling-quality = Upscaling quality
...
action-detect-imagery = Detect from imagery…
ai-title = Detect from aerial imagery
ai-intro = Runs a local model over the aerial imagery and places what it finds. Nothing leaves this machine, and nothing is written until you say so.
ai-model = Model
ai-model-hint = What to run. The list comes from ai.ron, next to the imagery configuration.
ai-model-places = Places: { $tags }
ai-model-missing = Weights not installed: { $file }
ai-no-models = No model configured
ai-no-backend = This build has no inference runtime — the vision crate was built without the onnx feature.
ai-failed = The detection stopped without a result — nothing was placed
ai-no-imagery = No imagery provider is configured
ai-no-track = The module has no track to measure a corridor from
ai-area = Where
ai-area-hint = The model only looks where you send it — which is what makes a run take a minute instead of an hour.
ai-area-track = Along the track
ai-area-drawn = In the drawn area
ai-area-none = No area drawn yet
ai-corridor = Out to
ai-corridor-hint = How far either side of the track to look.
ai-drawn = Area
ai-drawn-hint = Drawn with the AI area tool, or taken from the last circle the select tool grew.
ai-drawn-corners = { $corners } corners
ai-draw-area = Draw area
ai-keep-clear = Keep clear of track
ai-keep-clear-hint = Nothing is placed closer than this to a rail, whichever area was chosen.
ai-place-objects = Place what is found
ai-place-objects-hint = An object from the installed mods carrying the model's own tag — a car for a car, a lorry for a lorry.
ai-place-trees = Plant the trees
ai-place-trees-hint = What the model reads as a tree crown becomes a tree of the installed tree mods, in the species the model named and grown to the crown it measured. Selectable and deletable one by one afterwards, like any other tree.
ai-species = Species
ai-species-hint = Which trees the crowns become. "As detected" takes the model's own class and, where the model knows only one, the look of the crown itself — which at the resolution an imagery provider gives is a hint rather than a finding. Naming a stand overrules it for the whole run.
ai-species-detected = As detected
ai-tree-spacing = Keep trees apart
ai-tree-spacing-hint = How close a new tree may come to one that already stands there. It is what makes a second run over the same wood change nothing instead of doubling it.
ai-pave-lots = Pave the car parks
ai-pave-lots-hint = Cars standing in rows are a car park; it is paved as an unmarked asphalt area you can edit like a road afterwards.
ai-min-lot-cars = Cars per car park
ai-min-lot-cars-hint = How many cars have to stand together before they count as a car park rather than as parked cars.
ai-start = Run
ai-not-ready = Pick an installed model, and an area to run it on
ai-loading = Loading the model
ai-running = Window { $window } of { $windows } — { $found } found, { $tiles } tiles
ai-found = { $found } found
ai-found-lots = in { $lots } car parks
ai-cost = { $windows } windows, { $tiles } tiles
ai-no-objects = no object tagged { $tag } installed
ai-objects-installed = { $count } to choose from
status-ai-placed = { $objects } objects placed, { $trees } trees planted, { $lots } car parks paved
status-ai-area-draw = Click the corners of the area, Enter or right-click closes it
status-ai-area-set = Area of { $corners } corners set — open Detect from imagery again
status-ai-area-points = An area needs three corners
tool-group-ai = Imagery
tool-ai-area = AI area
tool-ai-area-hint = The patch of ground a model is let loose on: click the corners, Enter or right-click closes the ring
ai-open-dialog = Detect…
