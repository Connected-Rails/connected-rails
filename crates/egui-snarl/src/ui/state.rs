...
///
/// Connected Rails patch: lets the editor implement select-all and
/// select-after-paste — the crate keeps its selection state private otherwise.
pub fn set_selected_nodes(id: Id, ctx: &Context, nodes: &[NodeId]) {
    let selected = SelectedNodes(nodes.iter().copied().collect());
    selected.save(ctx, id);
    ctx.request_repaint();
}

/// Returns the currently selected nodes of the snarl with the given widget `id`.
