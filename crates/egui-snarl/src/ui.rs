    ...
    V: SnarlViewer<T>,
{
    ...
    // Connected Rails patch — blueprint-style bindings instead of `Scene`'s: the
    // secondary or middle button pans, the plain wheel zooms at the pointer, and
    // a primary drag on empty canvas draws the selection rect (see below).
    // Raw input on purpose: responses on this layer arrive mapped through the
    // very transform being edited here, which made them useless for it.
    let (pointer, pan_button, pointer_delta, scroll) = ui.ctx().input(|i| {
        (
    ...
    if let Some(pointer) = pointer
        && ui_rect.contains(pointer)
    {
        if pan_button && pointer_delta != Vec2::ZERO {
            to_global.translation += pointer_delta;
            snarl_resp.mark_changed();
        }
        if scroll != 0.0 {
            let new_scaling =
                (to_global.scaling * (scroll * 0.002).exp()).clamp(min_scale, max_scale);
            to_global = scale_transform_around(&to_global, new_scaling, pointer);
            snarl_resp.mark_changed();
        }
    }
    ...
    // Connected Rails patch: the rect starts on any primary drag over empty
    // canvas, no Shift needed — panning moved to the secondary button above.
    ...
    V: SnarlViewer<T>,
{
    ...
    // the node alone, Shift adds, Ctrl removes; dragging an unselected node
    // makes it the selection first, so the whole selection moves together.
    ...
    } else if r.dragged_by(PointerButton::Primary)
        && !modifiers.shift
        && !modifiers.command
